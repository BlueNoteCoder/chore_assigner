...
void assign_chores_to_people(std::vector<Chore> chores, std::map<std::string, Person>& people) {
    // Copy chore vector
...
int main() {
    std::vector<std::string> list_chores = {"Clean Living Room", "Vacuum Bed", "Sleep in tub"};
    std::vector<std::string> list_people = {"Steve", "Joe", "Ronny"};

    std::map<std::string, Person> my_people = create_many_people(list_people);
    std::vector<Chore> my_chores = create_many_chores(list_chores);


    assign_chores_to_people(my_chores, my_people);

    for (auto const& [key, val] : my_people) {
